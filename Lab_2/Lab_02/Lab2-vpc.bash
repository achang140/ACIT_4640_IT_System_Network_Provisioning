...
aws ec2 create-tags --resources "${vpc_cidr}" --tags Key=Name,Value=MyVPC --region $region

# enable dns hostname
aws ec2 modify-vpc-attribute --vpc-id "${vpc_cidr}" --enable-dns-hostnames Value=true

# Create public subnet
subnet_id=$(aws ec2 create-subnet --vpc-id "${vpc_cidr}" \
...
aws ec2 attach-internet-gateway --vpc-id "${vpc_cidr}" --internet-gateway-id $igw_id --region $region

# Create route table
route_table_id=$(aws ec2 create-route-table --vpc-id "${vpc_cidr}" \
...
ehco "key_name=${key_name}" >> infrastructure_data 